...
function createBaseState(userId: string, state: string, overrides?: Partial<BookConfirmationState>): BookConfirmationState {
  return {
    reviewData: {
...
describe("E2E: Manual Entry Flow", () => {
  ...
  it("Test 1: Complete manual entry → new book created → review saved", async () => {
    ...
    const state1 = createBaseState(userId, "awaiting_title");
    storeConfirmationState(userId, state1);

    const ctx1 = createMockContext(userId, title, 100) as Context;
    ...
    const ctx2 = createMockContext(userId, author, 100) as Context;
  ...
  it("Test 3: Cancel during title entry", async () => {
    ...
    const state = createBaseState(userId, "awaiting_title");
    storeConfirmationState(userId, state);

    // Verify state exists
    expect(getConfirmationState(userId)).not.toBeNull();

    const ctx = createMockContext(userId, "", 100) as Context;
  ...
  it("Test 4: Cancel during author entry", async () => {
    ...
    const state = createBaseState(userId, "awaiting_author", {
  ...
  it("Test 5: Empty/invalid inputs", async () => {
    ...
    const ctx1 = createMockContext(userId, "", 100) as Context;
    const handled1 = await handleTextInput(ctx1, botContext);
    ...
    const ctx2 = createMockContext(userId, "", 100) as Context;
    const handled2 = await handleTextInput(ctx2, botContext);
