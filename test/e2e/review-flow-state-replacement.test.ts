...
describe("E2E: Review State Replacement", () => {
  ...
  it("User starts new review while one pending → old state replaced automatically", async () => {
    ...
    const chatId = "1"; // Mock context uses chatId 1
    const ctx1 = createMockMessageContext(userId, fixture1.reviewText, 1) as Context;
    await handleReviewMessage(ctx1, botContext);

    // Verify first state exists (using user-based lookup since we don't know statusMessageId yet)
    const firstState = getConfirmationStateByUser(chatId, userId.toString());
    ...
    const secondState = getConfirmationStateByUser(chatId, userId.toString());
  ...
  it("Multiple sequential review attempts → only last one persists", async () => {
    ...
    let state = getConfirmationStateByUser(chatId, userId.toString());
    ...
    state = getConfirmationStateByUser(chatId, userId.toString());
